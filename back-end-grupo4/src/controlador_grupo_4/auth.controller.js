...
import User from '../model_grupo_4/user.model.js';


export const register = async (req, res) => {
  try {
    const { username, cedula, email, password } = req.body;
    ...
    if (existingUser2) {
      ...
      return res.status(400).json(['Ya existe un usuario con el mismo username'] );
    }
    ...
    const newUser = new User({ username, cedula, email, password: hashedPassword });
    await newUser.save();
